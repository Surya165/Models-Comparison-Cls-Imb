...
def segment(imagename,csvfile,mama):
	...
	for i in range(len(mitotic_imagelist)):
		cv.imwrite("segmented_data/mitotic/" + "img" +str(mitotic_list[i])+ str(i) + ".jpg",mitotic_imagelist[i])

	for j in range(len(nonmitotic_imagelist)):
		cv.imwrite("segmented_data/non_mitotic/" + "img"+str(nonmitotic_list[j]) +str(j) + ".jpg",nonmitotic_imagelist[j])
